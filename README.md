...
- Web Share API で Instagram などに画像共有(非対応端末はダウンロード + キャプションコピー)
...
| F-07 | Instagram シェア | Web Share API で PNG を共有シートへ渡す(非対応時は保存 + キャプションコピー) |
...
- [ ] Web Share 非対応環境では PNG が保存され、キャプションをコピーできる
